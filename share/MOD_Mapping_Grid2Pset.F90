...
!-----------------------------------------------------------------------
   ...
   CONTAINS
      ...
      procedure, PUBLIC :: map_varvalue_aweighted => map_g2p_varvalue_aweighted_2d
...
CONTAINS
   ...
   !-----------------------------------------------------
   SUBROUTINE map_g2p_varvalue_aweighted_2d (this, gdata, pdata)
         ...
         DO iproc = 0, p_np_worker-1
            IF (this%glist(iproc)%ng > 0) THEN
               ...
               DO ig = 1, this%glist(iproc)%ng
                  ...
                  ! if (gbuff(ig).gt.0) then
                     ! write(*, '(A, I4, F16.8)'),"LHB debug line900 withdraw error : ig, gbuff -----> ", gbuff(ig)
                  ! endif
               ENDDO
               
#ifdef USEMPI
               idest = p_address_worker(iproc)
               ! write(*,*),"LHB debug line906 withdraw error : iproc, idest, gbuff -----> ", iproc, idest, gbuff
               CALL mpi_send (gbuff, this%glist(iproc)%ng, MPI_DOUBLE, &
                  idest, mpi_tag_data, p_comm_glb, p_err)
      ...
      IF (p_is_worker) THEN
         ...
         DO iproc = 0, p_np_io-1
            IF (this%glist(iproc)%ng > 0) THEN
               ...
               allocate (pbuff(iproc)%val (this%glist(iproc)%ng))
               ! write(*,*),"LHB debug line926 withdraw error : iproc, pbuff -----> ", iproc, pbuff(iproc)%val
               ...
               CALL mpi_recv (pbuff(iproc)%val, this%glist(iproc)%ng, MPI_DOUBLE, &
                  isrc, mpi_tag_data, p_comm_glb, p_stat, p_err)
               ! write(*,*),"LHB debug line930 withdraw error : iproc, isrc, pbuff -----> ", iproc, isrc, pbuff(iproc)%val
#else
               pbuff(0)%val = gbuff
               deallocate (gbuff)
#endif
            ...
            IF (allocated(this%gweight(iset)%val)) THEN
               ...
               DO ig = 1, size(this%gweight(iset)%val)
                  ...
                  if (pdata(iset) /= spval) then 
                     pdata_tem(iset) = pdata_tem(iset) + pdata(iset) * pbuff(iproc)%val(iloc) * this%gweight(iset)%val(ig)
                     ! if(pbuff(iproc)%val(iloc).gt.0)then
                        ! WRITE(*, '(A, I4, I4, I4, F16.4)'),"LHB debug line946 withdraw error : ig, iproc, iloc, pbuff -----> ", ig, iproc, iloc, pbuff(iproc)%val(iloc)
                     ! endif
                  endif
                  ! if(pdata_tem(iset)<0)then
                  !    write(*,*) "LHB debug line955 withdraw error : pdata, pbuff, weight -----> ", pdata(iset), pbuff(iproc)%val(iloc), this%gweight(iset)%val(ig)
                  ! endif
               ENDDO
               ! if(pdata(iset).gt.0)then
                  ! WRITE(*, '(A, I4, F16.4)'),"LHB debug line946 withdraw error : iset, pdata -----> ", iset, pdata(iset)
                  ! WRITE(*, '(A, I4, F16.4)'),"LHB debug line946 withdraw error : iset, pdata_tem -----> ", iset, pdata_tem(iset)
               ! endif
            ELSE
               pdata_tem(iset) = spval
            ENDIF
         ENDDO
         pdata = pdata_tem
   ...
   END SUBROUTINE map_g2p_varvalue_aweighted_2d
