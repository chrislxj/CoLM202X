...
!
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_LAIFEEDBACK, DEF_USE_Fire, DEF_optimization_crop
   ...
   USE MOD_Vars_PFTimeVariables, only: lai_p, tlai_p, tsai_p, leafc_p, deadstemc_p, harvdate_p, slatop_opti_p
      ...
      DO m = ps, pe
         ...
         IF (ivt /= noveg) THEN
            ...
            IF(DEF_USE_LAIFEEDBACK)THEN
               ...
               if(DEF_optimization_crop)then
                  if (ivt >= npcropmin) then
                     tlai_p(m) = slatop_opti_p(m) * leafc_p(m)
                  endif
               endif
            ...
            ELSE IF (ivt >= npcropmin) THEN ! prognostic crops
               ...
               IF (harvdate_p(m) < 999 .and. tlai_p(m) < 1e-6) THEN
