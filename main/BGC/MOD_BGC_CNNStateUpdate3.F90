...
!                   2) Record accumulated fire-associated N transfers for veg and soil N semi-analytic spinup
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_NITRIF, DEF_USE_FIRE, DEF_USE_SASU, DEF_USE_DiagMatrix
   ...
   USE MOD_BGC_Vars_PFTimeVariables, only: &
             ...
             AKX_livestemn_to_deadstemn_p_acc, AKX_livecrootn_to_deadcrootn_p_acc
         ...
         ! patch-level wood to column-level litter (uncombusted wood)
            ...
            decomp_npools_vr(j,i_cel_lit,i) = decomp_npools_vr(j,i_cel_lit,i) &
             ...
             IF(DEF_USE_SASU .or. DEF_USE_DiagMatrix)THEN
                I_cwd_n_vr_acc(j,i) = I_cwd_n_vr_acc(j,i) + fire_mortality_to_cwdn(j,i) * deltim
                I_met_n_vr_acc(j,i) = I_met_n_vr_acc(j,i) + fire_mortality_to_met_n(j,i) * deltim
                I_cel_n_vr_acc(j,i) = I_cel_n_vr_acc(j,i) + fire_mortality_to_cel_n(j,i) * deltim
                I_lig_n_vr_acc(j,i) = I_lig_n_vr_acc(j,i) + fire_mortality_to_lig_n(j,i) * deltim
             ENDIF
          ENDDO
       ...
       ! litter and CWD losses to fire
          DO l = 1, ndecomp_pools
             DO j = 1, nl_soil
                ...
                IF(DEF_USE_SASU .or. DEF_USE_DiagMatrix)THEN
                   IF(l == i_met_lit) AKX_met_exit_n_vr_acc(j,i) = AKX_met_exit_n_vr_acc(j,i) + m_decomp_npools_to_fire_vr(j,l,i) * deltim
                   IF(l == i_cel_lit) AKX_cel_exit_n_vr_acc(j,i) = AKX_cel_exit_n_vr_acc(j,i) + m_decomp_npools_to_fire_vr(j,l,i) * deltim
                   IF(l == i_lig_lit) AKX_lig_exit_n_vr_acc(j,i) = AKX_lig_exit_n_vr_acc(j,i) + m_decomp_npools_to_fire_vr(j,l,i) * deltim
                   IF(l == i_cwd    ) AKX_cwd_exit_n_vr_acc(j,i) = AKX_cwd_exit_n_vr_acc(j,i) + m_decomp_npools_to_fire_vr(j,l,i) * deltim
                ENDIF
             ENDDO
          ENDDO
         ...
         DO m = ps , pe
           ...
           ! retranslocated N pool
            ...
            IF(DEF_USE_SASU .or. DEF_USE_DiagMatrix)THEN
               AKX_leafn_exit_p_acc      (m) = AKX_leafn_exit_p_acc      (m) + (m_leafn_to_fire_p      (m) + m_leafn_to_litter_fire_p      (m)) * deltim
               AKX_leafn_st_exit_p_acc   (m) = AKX_leafn_st_exit_p_acc   (m) + (m_leafn_storage_to_fire_p   (m) + m_leafn_storage_to_litter_fire_p   (m)) * deltim
               AKX_leafn_xf_exit_p_acc   (m) = AKX_leafn_xf_exit_p_acc   (m) + (m_leafn_xfer_to_fire_p   (m) + m_leafn_xfer_to_litter_fire_p   (m)) * deltim
               AKX_frootn_exit_p_acc     (m) = AKX_frootn_exit_p_acc     (m) + (m_frootn_to_fire_p     (m) + m_frootn_to_litter_fire_p     (m)) * deltim
               AKX_frootn_st_exit_p_acc  (m) = AKX_frootn_st_exit_p_acc  (m) + (m_frootn_storage_to_fire_p  (m) + m_frootn_storage_to_litter_fire_p  (m)) * deltim
               AKX_frootn_xf_exit_p_acc  (m) = AKX_frootn_xf_exit_p_acc  (m) + (m_frootn_xfer_to_fire_p  (m) + m_frootn_xfer_to_litter_fire_p  (m)) * deltim
               AKX_livestemn_exit_p_acc  (m) = AKX_livestemn_exit_p_acc  (m) + (m_livestemn_to_fire_p  (m) + m_livestemn_to_litter_fire_p  (m) + m_livestemn_to_deadstemn_fire_p(m)) * deltim
               AKX_livestemn_st_exit_p_acc(m) = AKX_livestemn_st_exit_p_acc(m) + (m_livestemn_storage_to_fire_p(m) + m_livestemn_storage_to_litter_fire_p(m)) * deltim
               AKX_livestemn_xf_exit_p_acc(m) = AKX_livestemn_xf_exit_p_acc(m) + (m_livestemn_xfer_to_fire_p(m) + m_livestemn_xfer_to_litter_fire_p(m)) * deltim
               AKX_deadstemn_exit_p_acc  (m) = AKX_deadstemn_exit_p_acc  (m) + (m_deadstemn_to_fire_p  (m) + m_deadstemn_to_litter_fire_p  (m)) * deltim
               AKX_deadstemn_st_exit_p_acc(m) = AKX_deadstemn_st_exit_p_acc(m) + (m_deadstemn_storage_to_fire_p(m) + m_deadstemn_storage_to_litter_fire_p(m)) * deltim
               AKX_deadstemn_xf_exit_p_acc(m) = AKX_deadstemn_xf_exit_p_acc(m) + (m_deadstemn_xfer_to_fire_p(m) + m_deadstemn_xfer_to_litter_fire_p(m)) * deltim
               AKX_livecrootn_exit_p_acc (m) = AKX_livecrootn_exit_p_acc (m) + (m_livecrootn_to_fire_p (m) + m_livecrootn_to_litter_fire_p (m) + m_livecrootn_to_deadcrootn_fire_p(m)) * deltim
               AKX_livecrootn_st_exit_p_acc(m) = AKX_livecrootn_st_exit_p_acc(m) + (m_livecrootn_storage_to_fire_p(m) + m_livecrootn_storage_to_litter_fire_p(m)) * deltim
               AKX_livecrootn_xf_exit_p_acc(m) = AKX_livecrootn_xf_exit_p_acc(m) + (m_livecrootn_xfer_to_fire_p(m) + m_livecrootn_xfer_to_litter_fire_p(m)) * deltim
               AKX_deadcrootn_exit_p_acc (m) = AKX_deadcrootn_exit_p_acc (m) + (m_deadcrootn_to_fire_p (m) + m_deadcrootn_to_litter_fire_p (m)) * deltim
               AKX_deadcrootn_st_exit_p_acc(m) = AKX_deadcrootn_st_exit_p_acc(m) + (m_deadcrootn_storage_to_fire_p(m) + m_deadcrootn_storage_to_litter_fire_p(m)) * deltim
               AKX_deadcrootn_xf_exit_p_acc(m) = AKX_deadcrootn_xf_exit_p_acc(m) + (m_deadcrootn_xfer_to_fire_p(m) + m_deadcrootn_xfer_to_litter_fire_p(m)) * deltim
               AKX_retransn_exit_p_acc    (m) = AKX_retransn_exit_p_acc    (m) + (m_retransn_to_fire_p    (m) + m_retransn_to_litter_fire_p    (m)) * deltim
               AKX_livestemn_to_deadstemn_p_acc(m)   = AKX_livestemn_to_deadstemn_p_acc(m)   + m_livestemn_to_deadstemn_fire_p(m) * deltim
               AKX_livecrootn_to_deadcrootn_p_acc(m) = AKX_livecrootn_to_deadcrootn_p_acc(m) + m_livecrootn_to_deadcrootn_fire_p(m) * deltim
            ENDIF
          ENDDO
      ENDIF
