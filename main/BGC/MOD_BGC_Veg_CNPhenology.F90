...
! !USES:
 ...
 ! crop variables
  ...
  !crop variables
       ...
       ntrp_soybean    , nirrig_trp_soybean, nnormal     , nirrig_normal     , &
       spval
...
! crop variables
       ...
       fertnitro_p       , plantdate_p         , maturitydate_p   , fert_p         ,&! input from files
#endif
...
! Xingjie Lu, 2021, revised the CLM5 code to be compatible with CoLM code sturcture.
   ...
   integer  h    ! 1 for north hemsiphere; 2 for south hemisphere
      ...
      ! phenology type - they depend only on onset_flag, offset_flag, bglfr, and bgtr
         ...
         CALL CNOffsetLitterfall(i,ps,pe,idate(1:3),deltim,npcropmin)
...
! !ARGUMENTS:
   ...
   !
   ! !LOCAL VARIABLES:
   integer :: ws_flag        !winter-summer solstice flag (0 or 1)
           ...
           ! test for switching from dormant period to growth period
            IF (dormant_flag_p(m) == 1) THEN
              ...
              ! before the growing degree-day summation starts again.
               ...
               IF (onset_gddflag_p(m) == 1 .and. ws_flag == 0) THEN
                  onset_gddflag_p(m) = 0
                  onset_gdd_p(m) = 0._r8
               ENDIF
              ...
              ! THEN accumulate growing degree days for onset trigger
               ...
               soilt = t_soisno(3,i)
               IF (onset_gddflag_p(m) == 1 .and. soilt > 273.15_r8) THEN
            ...
            ELSE IF (offset_flag_p(m) == 0) THEN
      ...
      DO m = ps , pe
        ...
        ! test for switching from dormant period to growth period
           ...
           ! for this dormancy period
               ...
               IF (onset_gddflag_p(m) == 0 .and. soilt < 273.15_r8) onset_fdd_p(m) = onset_fdd_p(m) + deltim/86400._r8
          ...
          ! IF soils are wet, accumulate soil water index for onset trigger
           ...
           ! Adding in Kyla's rainfall trigger when fun on. RF. prec10 (mm/s) needs to be higher than 8mm over 10 days.
              ...
              ! value, THEN override the onset_flag set from soil water.
                  ...
                  IF (onset_gddflag_p(m) == 1 .and. onset_gdd_p(m) < crit_onset_gdd) onset_flag_p(m) = 0
               ENDIF
  
           ! only allow onset IF dayl > 6hrs
               IF (onset_flag_p(m) == 1 .and. dayl(i) <= secspqtrday) THEN
                  onset_flag_p(m) = 0
               ENDIF
           ...
           ! as stress in offset soil water index
              ...
              ! THEN set flag to start the offset period and reset index variables
                  ...
                  IF (offset_swi_p(m) >= crit_offset_swi .and. onset_flag_p(m) == 0) offset_flag_p(m) = 1
           ...
           ! increase freezing day accumulator for cold soil
              ...
              ! IF freezing degree day sum is greater than critical value, initiate offset
                  IF (offset_fdd_p(m) > crit_offset_fdd .and. onset_flag_p(m) == 0) offset_flag_p(m) = 1
               ENDIF
      ...
      DO m = ps, pe
         ...
         IF(ivt >= npcropmin)THEN
           ...
           ! calculate gddmaturity
            IF(croplive_p(m))THEN
               ...
               IF (ivt == nswheat .or. ivt == nirrig_swheat .or. &
                   ...
                   ivt == nnormal .or. ivt == nirrig_normal)THEN
           ...
           ! --- --- ---
            ...
            onset_flag_p(m)  = 0 ! CN terminology to trigger certain
            offset_flag_p(m) = 0 ! carbon and nitrogen transfers
...
! Xingjie Lu, 2021, revised the CLM5 code to be compatible with CoLM code sturcture.
  ...
  ! !LOCAL VARIABLES:
   ...
   SUBROUTINE CNOffsetLitterfall(i,ps,pe,idate,deltim,npcropmin)
! !DESCRIPTION:
   ...
   integer  :: ivt, m
     ...
     ! only calculate fluxes during offset period
         IF (offset_flag_p(m) == 1) THEN
            ...
            IF (offset_counter_p(m) - deltim <= 1e-6) THEN
               t1 = 1.0_r8 / deltim
