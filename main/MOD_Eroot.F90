...
!-----------------------------------------------------------------------
   ...
   SUBROUTINE eroot (ipatch, nl_soil,trsmx0,porsl, &
  ...
  !=======================================================================
   ...
   USE MOD_Precision
   USE MOD_Const_Physical, only : tfrz, denh2o
#ifdef vanGenuchten_Mualem_SOIL_MODEL
  ...
  !-----------------------Argument-----------------------------------------
   integer, intent(in) :: ipatch             ! patch number
...
!       ENDDO
      ...
      roota = 1.e-10  
      do i = 1, nl_soil
         if(porsl(i)>=1.e-6 .and. t_soisno(i)>tfrz)then
#ifdef Campbell_SOIL_MODEL
      ...
      !write(*,*) "LHB debug line148 yield error : rstfac -----> ", rstfac
