...
!-----------------------------------------------------------------------
   ...
   real(r8), allocatable :: fertilizer_p(:)
   real(r8), allocatable :: manure_p(:)
      ...
      ELSEIF (DEF_FERT_SOURCE == 2) THEN
         IF (p_is_worker) THEN
            IF (numpft > 0) THEN
               ...
               allocate(fertilizer_p(numpft))
               allocate(manure_p(numpft))
            ENDIF
         ENDIF
         ...
         IF (p_is_worker) THEN
            DO ipft = 1, numpft
               IF (landpft%settyp(ipft) .ge. 15) THEN
                  manure_p(ipft) = manure_tmp(ipft)
                  IF (manure_p(ipft) < 0._r8) THEN
                     manure_p(ipft) = 0._r8
                  ENDIF
               ENDIF
            ENDDO
            ...
            IF (p_is_worker) THEN
               DO ipft = 1, numpft
                  IF (landpft%settyp(ipft) .eq. cft+14) THEN
                     fertilizer_p(ipft) = fertilizer_tmp(ipft)
                     IF (fertilizer_p(ipft) < 0._r8) THEN
                        fertilizer_p(ipft) = 0._r8
                     ENDIF
                  ENDIF
               ENDDO
            ENDIF
         ...
         ! fertnitro
         fertnitro_p(:) = fertilizer_p(:) + manure_p(:)

         IF (allocated (fertilizer_tmp)) deallocate(fertilizer_tmp)
         IF (allocated (manure_tmp)) deallocate(manure_tmp)
         IF (allocated (fertilizer_p)) deallocate(fertilizer_p)
         IF (allocated (manure_p)) deallocate(manure_p)
      ENDIF
