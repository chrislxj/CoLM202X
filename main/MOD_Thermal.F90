...
!---------------------Argument------------------------------------------
   ...
   integer , intent(in) :: c3c4 ! C3/C4 plant type
   real(r8), intent(in) :: &
...
IF ( patchtype==0.and.DEF_USE_LCT .or. patchtype>0 ) THEN
      ...
      IF (lai+sai > 1e-6) THEN
         ...
         CALL eroot (ipatch, nl_soil,trsmx0,porsl,&
#ifdef Campbell_SOIL_MODEL
            bsw,&
#endif
#ifdef vanGenuchten_Mualem_SOIL_MODEL
...
IF (DEF_USE_PFT .or. patchclass(ipatch)==CROPLAND) THEN
      ...
      DO i = ps, pe
         ...
         IF (lai_p(i)+sai_p(i) > 1e-6) THEN
            ...
            vmax_adjust = vmax25_p(p)
            effcon_adjust = effcon_p(p)
            if(DEF_optimization_crop)then
               if (p >= npcropmin) then
                  vmax_adjust = vmax25_opti_p(i)
                  effcon_adjust = effcon_opti_p(i)
               endif
            endif
   
            CALL LeafTemperature(ipatch,p,deltim,csoilc,dewmx   ,htvp       ,&
                 lai_p(i)   ,sai_p(i)   ,htop_p(i)  ,hbot_p(i)  ,sqrtdi_p(p),&
                 effcon_adjust,vmax_adjust,c3c4_p(p), slti_p(p) ,hlti_p(p)  ,shti_p(p)  ,&
